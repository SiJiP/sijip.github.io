...
submitButton.addEventListener("click", function() {
  if (!checkEmptyDate(startDate) || !checkEmptyDate(endDate)) {
    viewMessage(
      "class",
      "error-message",
      "Ops...Date field is empty. Please, enter the date!"
    );
  } else {
    ...
    newDate = new Date(
      ...
      startDateSplit[1] - 1,
      startDateSplit[2]
    ); /* newDate need for add date +1 day every iteration*/
    iteration = 0;
    timeoutCycle();
  }
});
...
function createChart() {
  Highcharts.chart("container-chart", {
    ...
    title: {
      text:
        "UAH to " + currency.value.toUpperCase() + " exchange rate over time"
    },
    subtitle: {
      text:
